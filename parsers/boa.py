...
def _find_last4(text: str):
    for p in [r'Card Number\s*[:\s]*\*+(\d{4})', r'Card\s+ending\s+in\s+(\d{4})', r'\*{2,}(\d{4})']:
...
def _find_amount(text: str):
    for p in [r'New Balance\s*[:\-]?\s*\$?\s*([0-9,]+\.\d{2})', r'Total Amount Due\s*[:\-]?\s*\$?\s*([0-9,]+\.\d{2})']:
        m = re.search(p, text, re.IGNORECASE)
        if m:
            return m.group(1)
    return None
...
def parse_text(text: str) -> dict:
    ...
    m = re.search(r'Bank of America\s+([A-Za-z0-9 &-]{3,40})', text)
    if m:
        out['card_name'] = ('Bank of America ' + m.group(1)).strip()
    elif 'bank of america' in text.lower() or 'boa' in text.lower():
        out['card_name'] = 'Bank of America'
